...
class Total_taxSerializer(serializers.ModelSerializer):
    class Meta:
        model = Total_tax
        fields = ['year_from', 'year_to', 'total_amount_of_taxes_collected', 'tag']
...
class Diding_taxesSerializer(serializers.ModelSerializer):
    class Meta:
        model = Diding_taxes
        fields = ['year_from', 'year_to', 'diding_taxes', 'tag']
...
class Rate_of_gdp_per_capita_growthSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['year_from', 'year_to', 'rate_of_gdp_per_capita_growth', 'tag']
...
class Annual_wagesSerializer(serializers.ModelSerializer):
    class Meta:
        model = Annual_wages
        fields = ['year_from', 'year_to', 'annual_wages', 'job_category', 'job_description', 'tag']

class Rate_of_returnSerializer(serializers.ModelSerializer):
    class Meta:
        model = Rate_of_return
        fields = ['year_from', 'year_to', 'rate_of_return', 'job_category', 'job_description', 'tag']

class Famine_eventSerializer(serializers.ModelSerializer):
    class Meta:
        model = Famine_event
        fields = ['year_from', 'year_to', 'famine_event', 'latitude', 'longitude', 'elevation', 'sub_category', 'magnitude', 'duration', 'tag']

class Disease_eventSerializer(serializers.ModelSerializer):
    class Meta:
        model = Disease_event
        fields = ['year_from', 'year_to', 'disease_event', 'latitude', 'longitude', 'elevation', 'sub_category', 'magnitude', 'duration', 'tag']

class Jinshi_degrees_awardedSerializer(serializers.ModelSerializer):
    class Meta:
        model = Jinshi_degrees_awarded
        fields = ['year_from', 'year_to', 'jinshi_degrees_awarded', 'emperor', 'population_in_year_x', 'tag']

class ExaminationSerializer(serializers.ModelSerializer):
    class Meta:
        model = Examination
        fields = ['year_from', 'year_to', 'examination', 'no_of_participants', 'degrees_awarded', 'passing_ratio', 'place', 'ratio_examiner_per_candidate', 'no_of_examiners', 'tag']

class Taiping_rebellionSerializer(serializers.ModelSerializer):
    class Meta:
        model = Taiping_rebellion
        fields = ['year_from', 'year_to', 'taiping_rebellion', 'rebel', 'place', 'ethnic_composition', 'family_background', 'role', 'rank', 'civil_examination', 'tag']

class Worker_wageSerializer(serializers.ModelSerializer):
    class Meta:
        model = Worker_wage
        fields = ['year_from', 'year_to', 'worker_wage', 'area', 'unskilled_construction', 'skilled_construction', 'number_of_districts_with_available_data', 'unskilled_arms_manufacturer', 'population_in_millions_in_1787', 'tag']

class PolitySerializer(serializers.ModelSerializer):
	crisisdb_population_related = PopulationSerializer(many=True, read_only=True)
	crisisdb_land_taxes_collected_related = Land_taxes_collectedSerializer(many=True, read_only=True)
	crisisdb_land_yield_related = Land_yieldSerializer(many=True, read_only=True)
	crisisdb_total_tax_related = Total_taxSerializer(many=True, read_only=True)
	crisisdb_total_economic_output_related = Total_economic_outputSerializer(many=True, read_only=True)
	crisisdb_total_revenue_related = Total_revenueSerializer(many=True, read_only=True)
	crisisdb_diding_taxes_related = Diding_taxesSerializer(many=True, read_only=True)
	crisisdb_salt_tax_related = Salt_taxSerializer(many=True, read_only=True)
	crisisdb_tariff_and_transit_related = Tariff_and_transitSerializer(many=True, read_only=True)
	crisisdb_misc_incomes_related = Misc_incomesSerializer(many=True, read_only=True)
	crisisdb_total_expenditure_related = Total_expenditureSerializer(many=True, read_only=True)
	...
	crisisdb_maritime_custom_related = Maritime_customSerializer(many=True, read_only=True)
	crisisdb_other_incomes_related = Other_incomesSerializer(many=True, read_only=True)
	crisisdb_revenue_official_related = Revenue_officialSerializer(many=True, read_only=True)
	crisisdb_revenue_real_related = Revenue_realSerializer(many=True, read_only=True)
	crisisdb_gdp_total_related = Gdp_totalSerializer(many=True, read_only=True)
	crisisdb_gdp_growth_rate_related = Gdp_growth_rateSerializer(many=True, read_only=True)
	crisisdb_shares_of_world_gdp_related = Shares_of_world_gdpSerializer(many=True, read_only=True)
	crisisdb_gdp_per_capita_related = Gdp_per_capitaSerializer(many=True, read_only=True)
	crisisdb_rate_of_gdp_per_capita_growth_related = Rate_of_gdp_per_capita_growthSerializer(many=True, read_only=True)
	crisisdb_wages_related = WagesSerializer(many=True, read_only=True)
	crisisdb_annual_wages_related = Annual_wagesSerializer(many=True, read_only=True)
	crisisdb_rate_of_return_related = Rate_of_returnSerializer(many=True, read_only=True)
	crisisdb_famine_event_related = Famine_eventSerializer(many=True, read_only=True)
	crisisdb_disease_event_related = Disease_eventSerializer(many=True, read_only=True)
	crisisdb_jinshi_degrees_awarded_related = Jinshi_degrees_awardedSerializer(many=True, read_only=True)
	crisisdb_examination_related = ExaminationSerializer(many=True, read_only=True)
	crisisdb_taiping_rebellion_related = Taiping_rebellionSerializer(many=True, read_only=True)
	crisisdb_worker_wage_related = Worker_wageSerializer(many=True, read_only=True)

	class Meta:
		model = Polity
		fields = ['id', 'name', 'start', 'end', 'crisisdb_population_related', 'crisisdb_land_taxes_collected_related', 'crisisdb_land_yield_related', 'crisisdb_total_tax_related', 'crisisdb_total_economic_output_related', 'crisisdb_total_revenue_related', 'crisisdb_diding_taxes_related', 'crisisdb_salt_tax_related', 'crisisdb_tariff_and_transit_related', 'crisisdb_misc_incomes_related', 'crisisdb_total_expenditure_related', 'crisisdb_balance_related', 'crisisdb_lijin_related', 'crisisdb_maritime_custom_related', 'crisisdb_other_incomes_related', 'crisisdb_revenue_official_related', 'crisisdb_revenue_real_related', 'crisisdb_gdp_total_related', 'crisisdb_gdp_growth_rate_related', 'crisisdb_shares_of_world_gdp_related', 'crisisdb_gdp_per_capita_related', 'crisisdb_rate_of_gdp_per_capita_growth_related', 'crisisdb_wages_related', 'crisisdb_annual_wages_related', 'crisisdb_rate_of_return_related', 'crisisdb_famine_event_related', 'crisisdb_disease_event_related', 'crisisdb_jinshi_degrees_awarded_related', 'crisisdb_examination_related', 'crisisdb_taiping_rebellion_related', 'crisisdb_worker_wage_related']
